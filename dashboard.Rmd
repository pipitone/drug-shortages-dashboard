...
```{r, fig.height=3}
weeks = tibble(
  week.start = seq(DSC_START, floor_date(now(), "month"), by = "1 week"), 
  i = 1
)
p = dsc.shortages %>%
  mutate(i = 1) %>%
...
### Shortages since March 2020
```{r}
dsc.shortages %>%
  mutate(start_date = date(coalesce(actual_start_date, anticipated_start_date))) %>%
  filter(start_date %within% interval("2020-03-01", "2020-03-31")) %>% 
...
### Shortage reasons, aggregated weekly {data-width=150}
```{r, fig.height=10, fig.width=3}
p = dsc.shortages %>%
  transmute(started = floor_date(actual_start_date, "week"), 
            reason = reason) %>%  
  filter(started > ymd('2020-01-01'), started < now()) %>%
  count(started, reason) %>%
  group_by(started) %>%
  mutate(percent = round(n / sum(n) * 100, 1)) %>%
  transmute(week_started = started, percent, reason) %>%
  ggplot(aes(x = week_started, y = percent)) + 
  geom_bar(stat='identity') + 
  labs(x=NULL, y="% of total") + 
  facet_wrap(~reason, ncol=1, strip.position="bottom")
ggplotly(p)
```
