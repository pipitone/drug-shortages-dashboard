...
DPD_DOWNLOAD = F # download the latest drug product database dataset
DSC_DOWNLOAD = F # download latest DSC database
...
time.by.month = tibble(
  month.start = seq(parse_date("2017-03-01"), parse_date("2020-01-01"), by = "1 month"), 
...
#    (month.start - as_date(actual_start_date)) / dyears(1) < 2) 
...
dsc.shortages %>% 
  ...
  ggplot(aes(y=percent,fill=reason)) + 
  xlim(0,3) + 
  geom_bar(aes(x=2.5), width=.5,stat="identity") + 
  coord_polar("y", clip="off") + 
  geom_text(aes(x=1.8, label = paste0(round(percent), "%")), position = position_stack(vjust = 0.5), size=4, check_overlap = T) +
