...
# date range to show 
FROM = floor_date(ymd("2019-09-01"), "week")
TO = as_date(now())
...
atc_group_plot = function(atc) {
  ...
  right_join(shortages, marketed, 
    ...
    ggplot(aes(x = month.date, y = count, linetype = fct_rev(status))) +
      geom_line()  + 
      labs(x=NULL, y = "# of DINs", linetype = NULL) + 
      expand_limits(y=0) +
    facet_wrap(~atc_name) + 
    theme(legend.position="bottom")
}
...
Row {data-height=300} 
------------------------------------- 
### Respiratory Beta Agonists (R03AC)
```{r}
atc_group_plot("R03AC")
```
...
### Quinolones (P01BA-)
```{r}
atc_group_plot("P01BA") 
```

Row 
-----------------------------------------------------------
### Shortage reports since March 2020
```{r}
dsc.shortages %>%
  mutate(start_date = date(coalesce(actual_start_date, anticipated_start_date))) %>%
  filter(start_date >= ymd("2020-03-01")) %>% 
