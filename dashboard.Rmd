...
dsc.shortages %>% 
  mutate(start_date = coalesce(actual_start_date, anticipated_start_date)) %>%
...
atc_group_plot = function(atcs) {
  ...
  right_join(shortages, marketed, 
    ...
    filter(sum(Available - Marketed) < 0) %>%
    mutate(percent_available = Available/max(Marketed)*100,
           percent_marketed = Marketed/max(Marketed)*100) %>%
    mutate(week = month.date) %>%
    left_join(atc_codes) %>%
    ggplot(aes(x = week, y = percent_available, fill=atc_name)) +
    geom_area()  + 
    scale_x_date(date_breaks="2 month", 
                 date_labels="%b, %Y", 
                 minor_breaks=NULL) + 
    scale_y_continuous(breaks =c(0,50,100)) + 
    facet_wrap(~atc_name, ncol=1, strip.position = "right") + 
...
Row {data-height=300} 
------------------------------------- 
### COVID-19 Indicator Medicines {data-width=150}
The figure below shows the medications from a list of medicines we are tracking to assess for COVID-related shortages. The figure shows medicines which have had shortages since November 2019.

```{r}
covid_meds = tibble(
  ATC =   c("R03AC", "N01A", "P01BA", "N02BE01"),
  Description = c("Respiratory Beta Agonists",  "Anesthetics",  "Quinolones", "Acetaminophen (Paracetamol)")
)
kable(covid_meds) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
...
### Searchable list of shortage reports since February, 2020
```{r}
dsc.shortages %>%
  mutate(start_date = date(coalesce(actual_start_date, anticipated_start_date))) %>%
  filter(start_date >= ymd("2020-02-01")) %>% 
  arrange(dpd_atc_number) %>%
  mutate(duration = round((start_date %--% estimated_end_date)/days(1)),
         start_date = format(start_date, "%b %d, %Y"),
         status = fct_recode(status, "Active" = "active_confirmed", "Anticipated" = "anticipated_shortage"),
         link = text_spec(id, link=paste0("https://www.drugshortagescanada.ca/shortage/", id))) %>%
  select(dpd_atc_number, en_drug_brand_name, drug_strength, company_name, status, start_date, duration, reason, link) %>%
  DT::datatable(escape=F, options = list(bPaginate = FALSE, 
                                         autoWidth = FALSE,
                                         rowid = FALSE), 
...
### Availability of COVID-19 Indicator medications  
```{r, fig.width=15, fig.height=4}
# ggplotly(atc_group_plot("R03AC"))
p = atc_group_plot(covid_meds$ATC)
p1 = p + geom_vline(xintercept=ymd("2020-01-15"), linetype=1) + 
    geom_vline(xintercept=ymd("2020-03-9"), linetype=1, colour="red") + 
    labs(x=NULL, y = "% available (by DIN)", 
         caption="Black line indicates first COVID-19 case in canada, red line indicates first death")  
p1
```
...
on [github](https://github.com/pipitone/drug-shortages-dashboard).
...
1. Why are there different totals "active shortages" and on the "drugs in shortage" graph?
